#FrancismarAlvesMartinsJunior_RM562869_fase2_cap7.R
...
#```{r}
# Variância
variancia <-var(dados$`Quantidade de Produtores`, na.rm = TRUE)
variancia
...
# Desvio padrão
desvio_padrao <- sd(dados$`Quantidade de Produtores`, na.rm = TRUE)
desvio_padrao
#```
...
# Desvio padrão
desvio_padrao <- sd(dados$`Produção por hectare (toneladas)`, na.rm = TRUE)
desvio_padrao
#```
